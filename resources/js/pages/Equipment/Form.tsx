...
import type { TypeOfEquipment, Brand, OperationalStatus, EquipmentDetail } from '@/types/inventory';
...
export default function EquipmentForm({ equipo, tiposEquipo, marcas, estados, onSubmit }: Props) {
    const { data, setData, processing, errors } = useForm<EquipmentFormData>({
        ...
        id_operational_status: equipo ? String(equipo.id_operational_status) : '',
    });
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid gap-6 sm:grid-cols-2">
                        <div className="grid gap-2">
                            <Label htmlFor="property_number">Número de inventario</Label>
                            <Input
                                id="property_number"
                        ...
                        <div className="grid gap-2">
                            ...
                            <Select
                                ...
                                onValueChange={(value) => setData('id_type_of_equipment', value)}
                            >
                                <SelectTrigger id="id_type_of_equipment" className="w-full">
                                    <SelectValue placeholder="Seleccionar..." />
                                </SelectTrigger>
                                <SelectContent>
                                    {tiposEquipo.map(t => (
                                        <SelectItem key={t.id_type_of_equipment} value={String(t.id_type_of_equipment)}>
                                            {t.type_of_equipment}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                        ...
                        <div className="grid gap-2">
                            ...
                            <Select
                                value={data.id_brand}
                                onValueChange={(value) => setData('id_brand', value)}
                            >
                                ...
                                <SelectContent>
                                    {marcas.map(m => (
                                        <SelectItem key={m.id_brand} value={String(m.id_brand)}>
                                            {m.name_brand}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                        ...
                        <div className="grid gap-2">
                            <Label htmlFor="number_workstation">Puesto de trabajo</Label>
                            <Input
                                id="number_workstation"
                                placeholder="Ej: PC-14"
                                value={data.number_workstation}
                                onChange={e => setData('number_workstation', e.target.value)}
                            />
                            <InputError message={errors.number_workstation} />
                        </div>

                        <div className="grid gap-2">
                            <Label htmlFor="id_operational_status">Estado operativo</Label>
                            <Select
                                value={data.id_operational_status}
                                onValueChange={(value) => setData('id_operational_status', value)}
                            >
                                <SelectTrigger id="id_operational_status" className="w-full">
                                    <SelectValue placeholder="Seleccionar..." />
                                </SelectTrigger>
                                <SelectContent>
                                    {estados.map(s => (
                                        <SelectItem key={s.id_operational_status} value={String(s.id_operational_status)}>
                                            {s.operational_status}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            <InputError message={errors.id_operational_status} />
                        </div>
                    </div>
